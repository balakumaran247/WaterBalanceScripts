...
#%% main
def run_SMBalance(MAIN_FOLDER,p_in,e_in,i_in,nrd_in,lu_in,smsat_file, start_year, end_year, 
...
# # #    comp = dict(zlib=True, complevel=9, least_significant_digit=2)
    ...
    etb.load().to_netcdf(etincr_path,encoding=encoding)
    del etb
    ...
    dsro.load().to_netcdf(dsro_path,encoding=encoding)
    del dsro
    ...
    sm.load().to_netcdf(sm_path,encoding=encoding)
    del sm
